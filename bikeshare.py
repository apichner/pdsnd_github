...
def get_filters():
    """
    Additional comments made to test
    Asks user to specify a city, month and day data to analyze.
    Returns:
        (str) city - name of the city to analyze
        (str) month - name of the month to filter by, or "all" to apply no month filter
        (str) day - name of the day of week to filter by, or "all" to apply no day filter
    """
    print('Hi! Let\'s explore some US bikeshare data! Note: only Chicago, NYC and Washington available.')
    MONTHS = ['january', 'february', 'march', 'april', 'may', 'june', 'all']
    WEEKDAYS = ['monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday', 'all']

    # User input for city : Chicago, New York City, Washington). Invalid inputs are captured with if loop.
    city = input('Enter name of city (Note: only Chicago, New York City, Washington are available at this time): ').lower()

    while True:
        if city in CITY_DATA:
            break
        else:
            print('Sorry, data is not available for that city')
            city = input('Enter name of a city (Note: only Chicago, New York City, Washington are available at this time): ').lower()

    # Get user input for month. Invalid inputs are captured using an if loop.
...
def station_stats(df):
    ...
    print('\nCalculating The Most Popular Stations and Trips : n')
    start_time = time.time()
    # Display most commonly used start station
    common_start_station = df['Start Station'].mode()[0]
    print('\nThe most common starting station was : ', common_start_station)
...
def user_stats(df):
    ...
    start_time = time.time()
    # Display count of user type
    user_type_count = df['User Type'].value_counts()
    print('\nThe user-type data breakdown is : \n', pd.DataFrame(user_type_count))
    # Display count of gender
    try:
        gender_count = df['Gender'].value_counts()
        print('\nThe gender data breakdown is : \n', pd.DataFrame(gender_count))
    except:
        print('\nAt this stage, no gender data is available for this dataset.')
    # Display earliest, most recent and most common year of birth
    try:
        oldest_year = df['Birth Year'].min()
        print('\nThe oldest user was born in the year : ', int(oldest_year))
        youngest_year = int(df['Birth Year'].max())
        print('\nThe youngest user was born in the year : ', int(youngest_year))
        common_year = int(df['Birth Year'].mode())
        print('\nThe year that most users were born in is : ', int(common_year))
    except:
...
def main():
    while True:
        ...
        user_stats(df)
        raw_data(df)
        restart = input('\nWould you like to restart the application? Please enter yes or no.\n')
